...
class OutLook(object):
    ...
    def get_mail(self):
        ...
        if self.argspec.search_sender:
            mails = mails.restrict(
                f"[SenderEmailAddress]='{self.argspec.search_sender}'")
        ...
        if len(mails) > 0:
            for fn in mails:
                ...
                if self.argspec.htmloutput:
                    ...
                    out = os.path.join(self.argspec.output,
                                       _get_safe_filename(out))
            ...
            try:
                for attachment in ent.Attachments:
                    if os.path.exists(os.path.join(output,
                                                   attachment.FileName)):  # 파일이름이 같은 경우
                        attachment.SaveASFile(
                            _get_safe_next_filename(
    ...
    def move_emails(self):
        if self.argspec.mfolder and self.argspec.maccount:  # 이동시킬 폴더의 계정과 폴더
            move = self.outlook.Folders(self.argspec.maccount).Folders(
                self.argspec.mfolder)
        else:  # 이동할 폴더의 계정을 지정하지않는다면 이동시킬 폴더의 계정을 사용함
            move = self.outlook.Folders(self.account).Folders(
                self.argspec.mfolder)
    ...
    def sendmail_with_csv(self):
        cc_list = []
        bcc_list = []
        recipients_list = []
        if self.argspec.stme:
            file_extension = os.path.splitext(self.argspec.stme)[1]
            if file_extension != '.csv':
                raise Exception(f"Invalid csv file")
            with open(self.argspec.stme, mode='r') as file:
                csvFile = csv.DictReader(file)
                for lines in csvFile:
                    _cc = \
                        list(filter(lambda x: x[0] == 'CC', lines.items()))[0][
                            1].split(', ')
                    _bcc = \
                        list(filter(lambda x: x[0] == 'BCC', lines.items()))[0][
                            1].split(', ')
                    _recipients = list(
                        filter(lambda x: x[0] == 'Recipients', lines.items()))[
                        0][1].split(', ')
                    if _cc[0]:
                        cc_list += _cc
                    if _bcc[0]:
                        bcc_list += _bcc
                    if _recipients[0]:
                        recipients_list += _recipients
        else:
            recipients_list = self.argspec.to
            cc_list = self.argspec.cc
            bcc_list = self.argspec.bcc
        return cc_list, bcc_list, recipients_list

    def sendmail(self):
        cc_list, bcc_list, recipients_list = self.sendmail_with_csv()
        mail = self.outlook.CreateItem(0)
        if not recipients_list:
            raise Exception("Mail To Required.")
        ...
        if self.argspec.htmlbody or self.argspec.htmlbody_file:
            if self.argspec.htmlbody_file:
                if not os.path.exists(self.argspec.htmlbody_file):
                    raise RuntimeError(
                        'Cannot find --body-file "%s"' % self.argspec.htmlbody_file)
            ...
            if self.argspec.key:
                for n, key in enumerate(self.argspec.key):
                    image_attachment = mail.Attachments.Add(self.argspec.value[n],
                                                            1, 0, key)
...
def _main(*args):
    with ModuleContext(
        ...
        mcxt.add_argument('--msubfolder', display_name='Target Subfolder',
                          ...
                          help='specify a subfolder')
        # ----------------------------------------------------------------------
        mcxt.add_argument('--stme',
                          input_method='fileread',
                          display_name='Send to multiple email',
                          input_group='Send Mails Conditions',
                          help='One or more email recipient with csv')
        # ----------------------------------------------------------------------
        ...
        # ----------------------------------------------------------------------
        mcxt.add_argument('--htmlbody-file',
                          input_group='Send Mails Conditions',
